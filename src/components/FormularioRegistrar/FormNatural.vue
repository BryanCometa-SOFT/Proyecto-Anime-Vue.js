    ...
    <div>
        <div class="d-flex align-items-center flex-column">
            <div class="form-group">
                <label class="text-white textoPequeno">Nombre</label>
                <input type="text" class="form-control inputPersonalizado"  required v-model="data.name">
            </div>

                <div class="form-group">
                <label class="text-white textoPequeno">Apellido</label>
                <input type="text" class="form-control inputPersonalizado" required  v-model="data.lastname">
            </div>
                ...
                <div class="form-group">
                <label class="text-white textoPequeno">Número de identificación</label>
                <input type="text" class="form-control inputPersonalizado" required v-model="data.identy_document" >
            </div>
            ...
            <div class="d-flex justify-content-center mt-n2" >
                <ButtonRosa texto="Registrar"  @click="registroUsuarioNatural()"/>
            </div>
        </div>
    </div>
</template>
...
export default {    
    ...
    methods:{
        ...
        //Guarda el usuario tipo Natural
        async registroUsuarioNatural(){
            if(this.data.name.length <= 0  || this.data.lastname.length <= 0  || this.data.telephone.length  <= 0 || this.data.identy_document.length  <= 0 || this.data.password.length  <= 0 || this.data.password_confirmation.length  <= 0 || this.data.email.length  <= 0 ){
                this.$swal({
                    title: 'Advertencia',
